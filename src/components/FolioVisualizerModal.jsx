    ...
    const triggerVerification = async () => {
        ...
        try {
            ...
            if (onFolioSync) {
                ...
                const originalFaltanNEO = _faltanNEO || [];
                const originalPhotos = PHOTOS || {};
                const originalExtrasCount = folioData.EXTRA_PHOTOS || 0;
                
                const statusChanged = currentStatus !== RESULTADO_AUDITORIA;
                const faltanNEOChanged = JSON.stringify(liveFaltanNEOList) !== JSON.stringify(originalFaltanNEO);
                const extrasCountChanged = extraFilesCount !== originalExtrasCount;
                const photosCountChanged = Object.keys(currentFoundPhotos).length !== Object.keys(originalPhotos).length;

                if (statusChanged || faltanNEOChanged || extrasCountChanged || photosCountChanged) {
