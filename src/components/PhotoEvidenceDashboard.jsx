...
const PhotoEvidenceDashboard = () => {
    ...
    const [firestoreOverrides, setFirestoreOverrides] = useState({});
    ...
    const filteredRecords = useMemo(() => {
        ...
        // Apply Firestore overrides in real-time
        filtered = filtered.map(r => {
            const override = firestoreOverrides[String(r.FOLIO)];
            if (override) {
                return {
                    ...r,
                    RESULTADO_AUDITORIA: override.status,
                    PHOTOS: override.photos,
                    _faltanNEO: override.faltanNEO,
        ...
        return filtered;
    }, [records, selectedStage, selectedCompany, selectedContract, selectedDelegation, firestoreOverrides]);
    ...
    // Listen for overrides/updates from anyone in the cloud for the active stage
    useEffect(() => {
        let q;
        if (selectedStage === 'ALL') {
            q = query(collection(firestoreDb, "audit_results"));
        } else {
            const currentStageVal = selectedStage === 'E3' && driveMode === 'SUPERVISOR' ? 'E3_SUP' : selectedStage;
            q = query(collection(firestoreDb, "audit_results"), where("stage", "==", currentStageVal));
        }
        
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setFirestoreOverrides(prev => {
                ...
                snapshot.docChanges().forEach((change) => {
                    ...
                    if (change.type === 'removed') {
                        delete newOverrides[folioId];
                    } else {
                        newOverrides[folioId] = {
                            status: data.status,
                            photos: data.photos ? data.photos.reduce((acc, p) => { acc[p.cat] = p; return acc; }, {}) : {},
                            faltanNEO: data.faltanNEO || []
                        };
                    }
                });
                return newOverrides;
